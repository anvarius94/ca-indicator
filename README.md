...
Так выглядит настоящий перехват. Адрес обычный, страница открывается, замок в браузере на месте —
но сертификат для `github.com` выдал не публичный центр, а корень антивируса, установленный на этом
компьютере. Значит трафик расшифровывается по дороге, и браузер об этом молчит.

<img src="screenshots/not-trusted-cert.png" width="400" alt="Сертификат github.com, выданный корнем антивируса вместо публичного центра">

Остальные состояния:

| Публичный УЦ | Нет шифрования | Недействительный сертификат | Разрешено вами |
|---|---|---|---|
| <img src="screenshots/public-ca.png" alt="Сертификат публичного УЦ с подписями Certificate Transparency"> | <img src="screenshots/no-encryption.png" alt="Страница, открытая по HTTP без шифрования"> | <img src="screenshots/invalid-cert.png" alt="Сертификат, забракованный браузером"> | <img src="screenshots/whitelisted.png" alt="Самоподписанный сертификат, разрешённый вручную для одного домена"> |
| Есть подписи CT, цепочка достроена до корня из Chrome Root Store | Страница по HTTP: данные идут открытым текстом | Просрочен, самоподписан или выдан не на этот домен | Ваш сервер: доверие выдано вручную и только на этом домене |
...
разницы между ними и тем, чего вы не разрешали. CA Trust Indicator показывает.
...
есть та самая лазейка, благодаря которой перехват вообще работает. CA Trust Indicator смотрит именно в
неё:
